...
namespace nanoFramework.Tools.FirmwareFlasher
{
    ...
    /// and use the snaphot as were it the online repository, by storing the snapshot in the firmware archive directory.
    ...
    public sealed class FirmwareArchiveManager
    {
            ...
            VerbosityLevel verbosity)
        {
            ...
            if (verbosity > VerbosityLevel.Normal)
            {
                ...
                Console.WriteLine($"Listing {platform} targets from firmware archive '{_archivePath}'...");
            }

            if (Directory.Exists(_archivePath))
            {
                foreach (var filePath in Directory.EnumerateFiles(_archivePath, $"*{_INFOFILE_EXTENSION}"))
                {
                    var packageInformation = JsonConvert.DeserializeObject<PersistedPackageInformation>(File.ReadAllText(filePath));
            ...
            VerbosityLevel verbosity)
        {
            ...
            CloudSmithPackageDetail remoteTarget = null;
            foreach (var isCommunityTarget in new bool[] { false, true })
            {
                foreach (var targetInfo in FirmwarePackage.GetTargetList(
                                                isCommunityTarget,
                                                preview,
                                                platform,
                                                verbosity))
                {
                    if (targetInfo.Name == targetName
                        && (version is null || version == targetInfo.Version))
                    {
                        remoteTarget = targetInfo;
                        break;
                    }
                }
            ...
            var (exitCode, fwFileName) = await package.DownloadPackageAsync(_archivePath, false, false);
            ...
            File.WriteAllText(
                Path.Combine(_archivePath, $"{fwFileName}{_INFOFILE_EXTENSION}"),
        ...
        private const string _INFOFILE_EXTENSION = ".json";
        #endregion
